...
def evolve(search,solver,duration,hae=[],types=True):
    ...
    while start+duration > time.time() :
        ...
        if hae_1[2]<hae_2[2] and dist < hae_2[2]:
            hae.pop(two)   
            hae.append([search.first, search.second,dist])
    min = float('inf')
    first,second = [],[]
...
def simulate(iterations,name):
    ...
    for _ in range(iterations):
        hae = []
        for _ in range(inner_itter):
            indexs = solver.get_random_sol()
            search = LocalSearch(indexs,solver.nodes,solver.dst_matrix,solver.dst_matrix_sorted,solver)
            search.steepest_list()
            hae.append([search.first,search.second,\
                sum(solver.count_new_dist(search.first))+sum(solver.count_new_dist(search.second))])
        results = evolve(search,solver,duration,hae)
        cycle_length = sum(solver.count_new_dist(results[0]))+sum(solver.count_new_dist(results[1]))
        if cycle_length < min_len:
                solver.save_fig([search.first,search.second],'hae_lp'+name)
                min_len = cycle_length
        dst_array_cycle.append(cycle_length)
    print('hae_lp_{}'.format(name),np.mean(dst_array_cycle),'(',np.min(dst_array_cycle),'-',np.max(dst_array_cycle),')')
    ...
    for _ in range(iterations):
        ...
        for _ in range(inner_itter):
            ...
            hae.append([search.first,search.second,\
                sum(solver.count_new_dist(search.first))+sum(solver.count_new_dist(search.second))])
        results = evolve(search,solver,duration,hae,types=False)
        cycle_length = sum(solver.count_new_dist(results[0]))+sum(solver.count_new_dist(results[1]))
        if cycle_length < min_len:
                solver.save_fig([search.first,search.second],'hae_no_lp'+name)
                min_len = cycle_length
        dst_array_cycle.append(cycle_length)
    print('hae_no_lp_{}'.format(name),np.mean(dst_array_cycle),'(',np.min(dst_array_cycle),'-',np.max(dst_array_cycle),')')
